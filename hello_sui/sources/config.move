module hello_sui::config {
    ...
    public fun add_mint_rule<R: drop>(
        ...
        vec_map::insert(&mut config.mint_rules, kind, type_name::get<R>(),);
    }

    public fun remove_mint_rule(_cap: &AdminCap, config: &mut Config, kind: u8) {
        vec_map::remove(&mut config.mint_rules, &kind,);
    }
